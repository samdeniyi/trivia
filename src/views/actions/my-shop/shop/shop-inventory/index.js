...
import {ReactComponent as BulkPriceIcon} from "./assets/bulk_price.svg";
...
const ShopInventory = ({
    ...
    useEffect(() => {
        inventory.length && setAllInventory(inventory.filter(
    ...
    useEffect(() => {
        ...
        if (filterType === "All products") {
            ...
            setAllInventory(inventory.filter(data => data.quantity > 0));
            setOpenFilterOptions(filterOpen => filterOpen === true && !filterOpen);
        ...
        ;
    }, [filterType, setAllInventory, inventory]);

    useEffect(() => {
    if(!inventory.length) return
        if (sortType === "Most Recent") {
            setAllInventory(inventory.sort((a, b) =>
    ...
    return (
        <Fragment>
            {!inventory.length? (
                <Fragment>
                    <TopHeader title={"Inventory"} withSpacesHeader backLink={"/actions/shop"}/>
                    <ScreenContainer>
                        <FlexSpaceBetweenBlock top={"64px"}>
                            ...
                            >
                                <RippleButton style={{bottom:"110px"}} type={"button"}>
                                    Add a product
                                </RippleButton>
                            </RippleLink>
                        </FlexSpaceBetweenBlock>
                    ...
                    >
                        ...
                        <List fullScreen>
                            {allInventories.map((product, index) => {
                                product.base64ProductImageString = product.base64ProductImageString || (product.images && product.images.baseImageUrl)
                            return(
                                <RippleLink
                                    key={index}
                                    to={{
				...
				items={[
					{
                        ...
                        link: "/actions/shop_products_add",
                        linkProp: shopId,
						click: () => {
					...
					},
					{
                        ...
                        link: "/actions/shop_inventory_edit",
                        linkProp: shopId,
						click: () => {
                            setOpenOptions(!openOptions);
                        }
